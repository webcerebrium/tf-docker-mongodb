locals {
    metrics_shortname = "mongodb-exporter"
    zone = var.zone
    ...
    metrics_service_port = 9216
    middleware_rewrite = "${local.metrics_shortname}-rewrite"
    middlewares = local.middleware_rewrite
    
    metrics_path = "/stats/mongodb/metrics"
    metrics_rewrite_path = "/stats/mongodb/"
}
